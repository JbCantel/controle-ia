...
\## 0. Modo de trabalho multiagente — Codex + Claude Code

Este projeto será continuado em **modo multiagente**, usando **Codex como agente principal/orquestrador** e **Claude Code como agente especialista**.

Esta seção define **quem faz o quê e como os dois colaboram**. Ela **não substitui** nenhuma regra técnica, decisão de produto, checkpoint, spec, plano ou restrição descrita nas seções seguintes. Em caso de conflito, as regras específicas e de segurança deste briefing prevalecem.

### 0.1. Hierarquia

- **Codex é o agente principal.**
- O usuário conversa normalmente com o Codex.
- O Codex lê o pedido, identifica as partes técnicas, visuais, de planejamento, conteúdo ou revisão e decide o que deve executar diretamente e o que deve delegar ao Claude.
- Claude é um **especialista chamado pelo Codex**, não um segundo orquestrador.
- O resultado do Claude volta para o Codex, que deve revisar, reconciliar com o código e com a documentação e só então decidir como utilizar.
- O Codex continua responsável pela integração final, testes, validação e estado do repositório.

### 0.2. Responsabilidades preferenciais do Codex

O Codex deve assumir principalmente:

- leitura técnica do repositório;
- arquitetura de implementação;
- React, Vite, Dexie/IndexedDB, React Router, Tailwind e demais dependências;
- banco de dados, schemas e migrações;
- componentes e páginas;
- design visual e implementação de UI;
- responsividade;
- acessibilidade técnica;
- performance;
- debugging;
- testes unitários, de banco e ponta a ponta;
- scripts e automações;
- Git, commits e integração final;
- validação de que a solução respeita este briefing, a spec e os planos;
- decisão final sobre como incorporar ao projeto as sugestões do Claude.

### 0.3. Responsabilidades preferenciais do Claude Code

Claude deve ser chamado principalmente para:

- planejamento e organização de etapas;
- revisão de requisitos e da spec;
- análise de produto e experiência do usuário;
- arquitetura de informação;
- microcopy, rótulos, mensagens, textos de interface e explicações;
- revisão de clareza dos checkpoints;
- revisão conceitual de UX;
- segunda opinião sobre fluxos, regras, casos-limite e riscos;
- revisão de um plano antes da implementação;
- conferir se uma proposta técnica deixou de atender alguma decisão documentada.

Claude pode ler código quando isso for necessário para responder corretamente, mas **por padrão trabalha em modo de análise/revisão e não edita arquivos**.

### 0.4. Pedidos mistos

Quando uma tarefa tiver partes de especialidades diferentes, o Codex deve dividi-la.

Exemplos:

- comportamento técnico + microcopy → Codex implementa; Claude revisa textos;
- nova tela + organização de conteúdo → Claude ajuda a estruturar; Codex desenha e implementa;
- regra complexa + UX → Codex valida e implementa a regra; Claude revisa a compreensão e a experiência;
- checkpoint visual → Codex analisa layout/implementação; Claude pode revisar hierarquia, clareza e experiência; o usuário toma a decisão final.

A colaboração não deve gerar duplicação desnecessária. Não é obrigatório que os dois opinem sobre toda tarefa pequena.

### 0.5. Regra de edição

Para evitar conflitos:

- **Codex é o editor principal do repositório.**
- Claude, por padrão, não modifica arquivos quando chamado pelo Codex.
- Claude devolve recomendações, textos, análise ou plano.
- Codex valida e faz a alteração.
- Nunca deixe Codex e Claude editarem simultaneamente o mesmo arquivo.
- Se o usuário pedir explicitamente que Claude implemente algo, Codex deve delimitar arquivos/tarefa antes da delegação e evitar edição concorrente.

### 0.6. Como chamar Claude

Quando precisar do Claude, o Codex deve usar o Claude Code instalado localmente e trabalhar no mesmo repositório/contexto.

A chamada deve fornecer:

1. objetivo exato;
2. arquivos/documentos que Claude deve ler;
3. restrições relevantes;
4. formato esperado da resposta;
5. indicação explícita de que, salvo ordem contrária, não deve alterar arquivos.

O Codex não deve delegar de forma vaga como “analise tudo”. A tarefa deve ser delimitada.

### 0.7. Disponibilidade e fallback

Antes da primeira delegação da sessão, o Codex deve verificar se o Claude Code está acessível.

- **Codex + Claude disponíveis:** usar normalmente o modo multiagente.
- **Claude indisponível:** informar claramente que a colaboração não pôde ser executada. Para este primeiro teste multiagente, não esconder a falha nem fingir que houve consulta ao Claude.
- Depois que o teste estiver validado, o Codex pode assumir sozinho tarefas do Claude quando este estiver indisponível, desde que avise.
- Se o Codex estiver indisponível, esta orquestração não se aplica automaticamente; o usuário decide se quer continuar somente com Claude.

### 0.8. Controle explícito do usuário

Uma instrução explícita do usuário sempre vence o roteamento automático:

- `[CODEX]` ou “somente Codex” → Codex trabalha sozinho e não chama Claude.
- `[CLAUDE]` ou “somente Claude” → a tarefa deve ser direcionada ao Claude; Codex atua apenas como ponte se necessário.
- `[TEAM]` ou “use os dois” → Codex coordena e deve envolver Claude de forma real.
- Sem marcador → Codex decide pela intenção da tarefa, respeitando as responsabilidades acima.

### 0.9. Fonte de verdade

Os agentes devem compartilhar a mesma fonte de verdade:

1. este briefing;
2. a spec vigente;
3. os planos já aprovados;
4. o código atual;
5. os testes;
6. decisões explícitas do usuário.

Nenhum agente deve criar um “processo paralelo” que contradiga esses documentos.

Se Claude sugerir algo incompatível com a documentação ou com o código, o Codex deve apontar a divergência em vez de aplicar silenciosamente.

### 0.10. Primeiro teste multiagente obrigatório

Na primeira sessão após receber este briefing, **não altere nenhum arquivo**.

O teste deve funcionar assim:

#### Parte A — Codex

Codex deve:

1. ler este briefing inteiro;
2. ler a spec e o Plano B indicados abaixo;
3. inspecionar o repositório e a branch atual;
4. verificar o estado técnico;
5. rodar os testes e o build seguros previstos neste briefing;
6. identificar o checkpoint atual e os próximos passos.

#### Parte B — Claude

Depois, Codex deve chamar Claude em modo somente leitura e pedir que ele:

1. leia este briefing;
2. leia a spec vigente e o Plano B;
3. faça uma revisão independente do estado documentado;
4. identifique decisões que não podem ser perdidas;
5. identifique riscos e casos-limite para a continuação;
6. revise a ordem proposta para as próximas etapas;
7. não altere nenhum arquivo.

#### Parte C — Reconciliação pelo Codex

Codex deve comparar a própria análise com a resposta do Claude e apresentar ao usuário um único relatório contendo:

- `Codex consultado: sim`;
- `Claude consultado: sim` ou o erro real encontrado;
- estado técnico encontrado;
- divergências entre código e briefing, se houver;
- pontos em que Codex e Claude concordaram;
- pontos em que houve diferença de avaliação;
- riscos antes de continuar;
- próximo checkpoint;
- perguntas que precisam da decisão do usuário.

**Não iniciar a Etapa 4 durante esse teste.**

O briefing já determina que o Checkpoint 2 precisa ser resolvido antes das telas seguintes. Essa regra continua valendo integralmente.

### 0.11. Princípio de colaboração

O objetivo não é fazer duas IAs produzirem o mesmo trabalho.

O objetivo é:

**Codex coordenar e implementar + Claude planejar/revisar onde agrega valor + Codex integrar e validar.**

Use o segundo agente quando a participação dele melhorar materialmente a tarefa. Evite chamadas apenas para “cumprir tabela”, exceto no primeiro teste obrigatório acima.
...
\- \*\***Cor:**\*\* paleta fixa \`CATEGORY\_PALETTE\` (\`domain/categories.js\`).
